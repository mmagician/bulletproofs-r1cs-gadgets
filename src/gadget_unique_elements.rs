...
mod tests {
    ...
    pub fn gen_proof_of_uniqueness<R: RngCore + CryptoRng>(
        set: &[u64],
        mut rng: &mut R,
        ...
        for i in 0..set.len() {
            for j in i + 1..set.len() {
                ...
                let (comm_diff_inv, var_diff_inv) =
                ...
                let set_lc: LinearCombination = vec![
                    (var_diff, Scalar::one()),
                    (set_variables[j], Scalar::one()),
                    (set_variables[i], -Scalar::one()),
                ]
                .iter()
                .collect();
    ...
    pub fn verify_proof_of_uniqueness(
        set_length: usize,
        proof: R1CSProof,
        ...
        assert!(
            diff_commitments.len() == diff_inv_commitments.len(),
            "Ooops, differences & inverses vectors not equal!"
        );
